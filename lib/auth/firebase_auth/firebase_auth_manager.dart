    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Прошло много времени с последнего входа. Войдите ещё раз перед удалением Вашего аккаунта')),
        );
      }
    }
  }
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Прошло много времени с последнего входа. Войдите ещё раз перед обновлением вашего Email')),
        );
      }
    }
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : JeepingFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Неверный Email или пароль')),
